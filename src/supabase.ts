...
export async function gradeLongAnswer(question:Question, answer:string):Promise<AIGrade>{
  if(!hasSupabase)return localGrade(question,answer);
  try{
    ...
    if(!response.ok)throw new Error('Falha ao chamar avaliador');
...
export async function loadQuestions():Promise<Question[]>{
  if(hasSupabase){
    try{
      ...
      if(response.ok){const rows=await response.json() as {payload:Question}[];if(rows.length)return rows.map(r=>r.payload)}
    }catch(error){console.warn('Banco indisponível; usando perguntas locais.',error)}
  }
  const response=await fetch('assets/data/questions.json');return await response.json() as Question[];
}

export async function saveAttempt(score:number, answers:unknown[]){
  ...
  }catch(error){console.warn('Não foi possível salvar tentativa.',error)}
}
