...
export function initSignalAndBuild() {
    ...
    const run = document.querySelector('#runSignal');
    const light = document.querySelector('#lightValue');
    const sensor = document.querySelector('#sensorValue');
    const code = document.querySelector('#codeValue');
    const log = document.querySelector('#signalLog');
    const stages = [...document.querySelectorAll('.signal-stage')];
    let busy = false;
    ...
    run?.addEventListener('click', async () => {
        ...
        if (log)
            log.textContent = '[01] Fonte luminosa ativada → propagação iniciada';
        stages[0]?.classList.add('active');
        if (light)
            light.textContent = String(520 + Math.round(Math.random() * 45));
        await wait(850);
        stages[0]?.classList.remove('active');
        stages[1]?.classList.add('active');
        const raw = 580 + Math.round(Math.random() * 330);
        if (sensor)
            sensor.textContent = String(raw);
        if (log)
            log.textContent = `[02] Fotossensor converteu luz → leitura ADC ${raw}`;
        await wait(850);
        stages[1]?.classList.remove('active');
        stages[2]?.classList.add('active');
        if (code)
            code.textContent = raw > 710 ? 'ALTO' : 'MÉDIO';
        if (log)
            log.textContent = `[03] Código comparou o valor → estado ${raw > 710 ? 'ALTO' : 'MÉDIO'}`;
        await wait(900);
        stages[2]?.classList.remove('active');
        consoleEl?.classList.remove('running');
        busy = false;
    });
    const partInfo = document.querySelector('#partInfo');
    document.querySelectorAll('[data-part]').forEach(button => button.addEventListener('click', () => {
        ...
        if (partInfo)
            partInfo.textContent = parts[key] || '';
    }));
